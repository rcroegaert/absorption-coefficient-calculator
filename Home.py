...
from src import utils, models, absorption_coeff
...
with st.expander('Werte ein/ausblenden...'):
    ...
    for i, column in enumerate(columns):
        ...
        value2 = column.selectbox(f"Modell", options=['Bitte wählen Sie', 'Poröser', 'Lochplatte', 'Luft'], key=f"value2_{i}")
...
abs_coeff = absorption_coeff.AbsorptionCoeff(L1, L2, f_start, f_end, [por_abs],
                                             luft_dichte, luft_c, 0)
alphas = abs_coeff.absorption_coeffs()


################## Output Section ##################
# Plotting
st.header('Plot :bar_chart:')
titlestr = ('Absorptionsgrad eines {} mm Material'.format(material_dict["Material 1"][0]) + ' bei {} mm Luftspalt'.format(material_dict["Material 2"][0]))
